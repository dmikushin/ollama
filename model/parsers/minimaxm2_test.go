...
var cmpToolCalls = cmp.Options{
	cmpopts.IgnoreUnexported(api.ToolCallFunctionArguments{}, api.ToolPropertiesMap{}),
}

// helpers for v0.19 ordered-map API types

func makeProps(entries ...any) *api.ToolPropertiesMap {
	m := api.NewToolPropertiesMap()
	for i := 0; i < len(entries)-1; i += 2 {
		m.Set(entries[i].(string), entries[i+1].(api.ToolProperty))
	}
	return m
}

func makeArgs(entries ...any) api.ToolCallFunctionArguments {
	a := api.NewToolCallFunctionArguments()
	for i := 0; i < len(entries)-1; i += 2 {
		a.Set(entries[i].(string), entries[i+1])
	}
	return a
}
	...
	tools := []api.Tool{
		{
			...
			Function: api.ToolFunction{
				...
				Parameters: api.ToolFunctionParameters{
					Type: "object",
					Properties: makeProps("location", api.ToolProperty{Type: api.PropertyType{"string"}}, "unit", api.ToolProperty{Type: api.PropertyType{"string"}}),
				},
			},
		},
		{
			...
			Function: api.ToolFunction{
				...
				Parameters: api.ToolFunctionParameters{
					Type: "object",
					Properties: makeProps("query", api.ToolProperty{Type: api.PropertyType{"string"}}, "limit", api.ToolProperty{Type: api.PropertyType{"integer"}}),
				},
			},
		},
		{
			...
			Function: api.ToolFunction{
				...
				Parameters: api.ToolFunctionParameters{
					Type: "object",
					Properties: makeProps("expression", api.ToolProperty{Type: api.PropertyType{"string"}}),
				},
			},
		},
	}
			...
			wantCalls: []api.ToolCall{
				{
					Function: api.ToolCallFunction{
						Name: "search_web",
						Arguments: makeArgs("query", map[string]any{"keywords": []any{"AI", "news"}}),
					},
				},
			},
		},
			...
			wantCalls: []api.ToolCall{
				{
					Function: api.ToolCallFunction{
						Name: "search_web",
						Arguments: makeArgs("query", []any{"AI", "machine learning"}),
					},
				},
			},
		},
			...
			wantCalls: []api.ToolCall{
				{
					Function: api.ToolCallFunction{
						Name: "search_web",
						Arguments: makeArgs("query", "AI news",
							"limit", float64(10)),
					},
				},
			},
			...
			wantCalls: []api.ToolCall{
				{
					Function: api.ToolCallFunction{
						Name: "search_web",
						Arguments: makeArgs("query", "AI &amp; Machine Learning",),
					},
				},
			},
		},
...
Tokyo
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tt.wantCalls, gotCalls, cmpToolCalls...); diff != "" {
...
func TestMiniMaxM2ParserStreaming(t *testing.T) {
	tools := []api.Tool{
		{
			...
			Function: api.ToolFunction{
				...
				Parameters: api.ToolFunctionParameters{
					Type: "object",
					Properties: makeProps("location", api.ToolProperty{Type: api.PropertyType{"string"}}),
				},
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tt.wantCalls, allCalls, cmpToolCalls...); diff != "" {
